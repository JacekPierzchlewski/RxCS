...
def main(dAcqConf, dSig):
    ...
    WARNING: Due to different number of sampling points in patterns generated with JS scheme,
...
def _printConf(dAcqConf, dSig):
    ...
    if bMute == 0:
        ...
        rxcs.console.progress('Signal sampling', 'Nonuniform sampler (JS)')
...
def _generate_patterns(dAcqConf, dSig):
    ...
    for inxP in np.arange(nPatts):   #<- loop over all patterns
        ...
        vPattern = _js_engine(nK_s, nT, nK_g, iSigma)
...
# JS engine
# =================================================================
def _js_engine(nK_s, nT, K_g, sigma):
